...
function Server(compiler, options) {
	...
	this.listeningApp.once("close", function() {
		this.middleware.close();
	}.bind(this));

	// Proxy events from `listeningApp` to anyone who is interested.
	var emit = this.listeningApp.emit;
	this.listeningApp.emit = function() {
		emit.apply(this.listeningApp, arguments);
...
Server.prototype.address = function() {
	return this.listeningApp.address();
}
